...
class DX_Auth
{
    ...
    */
    ...
    'smtp_port' => 465,
    'smtp_user' => 'royaltech.info@gmail.com',
    'smtp_pass' => 'masoora1',
);
    ...
    {
        if ($this->ci->config->item('DX_count_login_attempts') and !$this->
    ...
    function _create_autologin($user_id)
    {
        ...
        $user = array('key_id' => substr(md5(uniqid(rand() . $this->ci->input->cookie($this->
            ci->config->item('sess_cookie_name')))), 0, 16), 'user_id' => $user_id);
    ...
    function autologin()
    {
        ...
        if ($auto = $this->ci->input->cookie($this->ci->config->item('DX_autologin_cookie_name')) and
            !$this->ci->session->userdata('DX_logged_in'))
        {
            // Extract data
    ...
    function _set_session($data)
    {
        ...
        $user = array('DX_user_id' => $data->id, 'DX_username' => $data->username,
            'DX_role_id' => $data->role_id, 'DX_role_name' => $role_data['role_name'],
            ...
            'DX_permission' => $role_data['permission'], 'DX_parent_permissions' => $role_data['parent_permissions'],
            'DX_logged_in' => true);
    ...
    function _auto_cookie($data)
    {
        ...
        $cookie = array('name' => $this->ci->config->item('DX_autologin_cookie_name'),
            'value' => serialize($data), 'expire' => $this->ci->config->item('DX_autologin_cookie_life'));
    ...
    */
    ...
    function is_max_login_attempts_exceeded()
    {
        ...
        return ($this->ci->login_attempts->check_attempts($this->ci->input->ip_address())->
            num_rows() >= $this->ci->config->item('DX_max_login_attempts'));
    }
    ...
    function login($login, $password, $remember = true)
    {
        ...
        if (!empty($login) and !empty($password))
        {
            ...
            if ($this->ci->config->item('DX_login_using_username') and $this->ci->config->
                item('DX_login_using_email'))
            {
                ...
                // Get user query
                if ($query = $this->ci->users->$get_user_function($login) and $query->num_rows() ==
                    1)
                {
                    // Get user record
                    $row = $query->row();
            ...
            // Check if login is still not activated
                elseif ($query = $this->ci->user_temp->$get_user_function($login) and $query->
                    num_rows() == 1)
                {
                    // Set error message
    ...
    function register($username, $password, $email)
    {
        ...
        // New user array
        $new_user = array('username' => $username, 'password' => crypt($this->_encode($password)),
            'email' => $email, 'last_ip' => $this->ci->input->ip_address());
        ...
        if ($insert)
        {
            ...
            if ($this->ci->config->item('DX_email_activation'))
            {
                ...
                $subject = sprintf($this->ci->lang->line('auth_activate_subject'), $this->ci->
                    config->item('DX_website_name'));

                // Activation Link
                $new_user['activate_url'] = site_url($this->ci->config->item('DX_activate_uri') .
                    "{$new_user['username']}/{$new_user['activation_key']}");
            ...
            } else
            {
                ...
                if ($this->ci->config->item('DX_email_account_details'))
                {
                    // Create email
                    $from = $this->ci->config->item('DX_webmaster_email');
                    $subject = sprintf($this->ci->lang->line('auth_account_subject'), $this->ci->
                        config->item('DX_website_name'));
        ...
        if ($login)
        {
            ...
            if ($query = $this->ci->users->get_login($login) and $query->num_rows() == 1)
            {
                ...
                if (!$row->newpass_key)
                {
                    ...
                    $data['reset_password_uri'] = site_url($this->ci->config->item('DX_reset_password_uri') .
                        "{$row->username}/{$data['key']}");
    ...
    function reset_password($username, $key = '')
    {
        ...
        // Get user id
        if ($query = $this->ci->users->get_user_by_username($username) and $query->
            num_rows() == 1)
        {
            $user_id = $query->row()->id;

            // Try to activate new password
            if (!empty($username) and !empty($key) and $this->ci->users->activate_newpass($user_id,
                $key) and $this->ci->db->affected_rows() > 0)
            {
    ...
    function activate($username, $key = '')
    {
        ...
        // Activate user
        if ($query = $this->ci->user_temp->activate_user($username, $key) and $query->
            num_rows() > 0)
        {
            // Get user
            $row = $query->row_array();
    ...
    function change_password($old_pass, $new_pass)
    {
        ...
        if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata('DX_user_id')) and
            $query->num_rows() > 0)
        {
            ...
            if (crypt($pass, $row->password) === $row->password)
            {
                ...
                // Trigger event
                $this->ci->dx_auth_event->user_changed_password($this->ci->session->userdata('DX_user_id'),
                    $new_pass);
    ...
    function captcha()
    {
        ...
        $vals = array('img_path' => './' . $captcha_dir . '/', 'img_url' => base_url() .
            $captcha_dir . '/', 'font_path' => $this->ci->config->item('DX_captcha_fonts_path'),
            'font_size' => $this->ci->config->item('DX_captcha_font_size'), 'img_width' => $this->
            ci->config->item('DX_captcha_width'), 'img_height' => $this->ci->config->item('DX_captcha_height'),
            'show_grid' => $this->ci->config->item('DX_captcha_grid'), 'expiration' => $this->
            ci->config->item('DX_captcha_expire'));
    ...
    function is_captcha_expired()
    {
        ...
        // Check if captcha already expired
        return (($this->ci->session->flashdata('captcha_time') + $this->ci->config->
            item('DX_captcha_expire')) < $now);
    }
    ...
    function get_recaptcha_switch_image_audio_link($switch_image_text =
        'Get an image CAPTCHA', $switch_audio_text = 'Get an audio CAPTCHA')
    {
        return '<div class="recaptcha_only_if_image"><a href="javascript:Recaptcha.switch_type(\'audio\')">' .
            $switch_audio_text . '</a></div>
			<div class="recaptcha_only_if_audio"><a href="javascript:Recaptcha.switch_type(\'image\')">' .
            $switch_image_text . '</a></div>';
    }

    function get_recaptcha_label($image_text = 'Enter the words above', $audio_text =
